...
using Standards.Net.Exceptions;

namespace Standards.Net.API.Filters;

/// <summary>
/// Global exception handling endpoint filter.
/// Maps DomainException types to appropriate HTTP status codes with consistent response format.
/// </summary>
public sealed class ExceptionHandlingEndpointFilter : IEndpointFilter
{
    ...
    )
    {
        ...
        catch (DomainException ex)
        {
            ...
            return Results.Json(response, statusCode: (int)ex.Code);
        }
        ...
        catch (InvalidOperationException ex)
        {
            ...
            return Results.BadRequest(
                ApiResponse<object>.Error(ex.Message, new List<string> { ex.Message })
            );
        }
        ...
        catch (FormatException ex)
        {
            ...
            return Results.BadRequest(
                ApiResponse<object>.Error("Invalid data format", new List<string> { ex.Message })
            );
        }
        catch (Exception ex)
        {
